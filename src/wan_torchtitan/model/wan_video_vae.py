...
def check_is_instance(model, module_class):
    ...
    if hasattr(model, "module") and isinstance(model.module, module_class):
        return True
    # FSDP might assume `module` is the wrapped module, but also check `_fsdp_wrapped_module` if needed? 
    # Usually `.module` is exposed by DDP/FSDP wrappers or conventions.
    return False
...
class CausalConv3d(nn.Conv3d):
    ...
    def forward(self, x, cache_x=None):
        # TODO: zirui, fixed Input type (double) and bias type (float) mis-match issue 
        ...
        if cache_x is not None and self._padding[4] > 0:
            cache_x = cache_x.to(device=device, dtype=dtype)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, dim):
        ...
        # nn.init.zeros_(self.proj.weight) # Moved to reset_parameters

    def reset_parameters(self):
        # We need to call reset_parameters on submodules if we want them to re-init
        # But usually FSDP/Meta handles submodules. 
        # However, we have specific custom init logic here:
        for module in self.children():
            if hasattr(module, 'reset_parameters'):
                module.reset_parameters()
        
        # Apply specific custom init
...
class VideoVAE_(nn.Module):
    ...
    def encode(self, x, scale):
        ...
        for i in range(iter_):
            feat_idx = 0
            if i == 0:
                out, feat_cache, feat_idx = self.encoder(x[:, :, :1, :, :], feat_cache=feat_cache, feat_idx=feat_idx)
            else:
